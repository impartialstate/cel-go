...
// runtime cost calculations in track.go.
...
// NewAstNode returns an AstNode for a value of the given type which is reached by the given field
// path.
//
// Cost estimators use it to describe values which do not appear in the expression on their own,
// such as the elements of a list which was passed to a function.
func NewAstNode(path []string, t *types.Type, expr ast.Expr, size *SizeEstimate) AstNode {
	return astNode{path: path, t: t, expr: expr, derivedSize: size}
}
