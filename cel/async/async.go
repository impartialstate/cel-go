...
// its own goroutine and TimeoutBinding selects on the deadline, returning a timeout error when it
...
// are not under the caller's control. The extra goroutine is incurred only by TimeoutBinding-
// wrapped calls, not by async evaluation in general.
